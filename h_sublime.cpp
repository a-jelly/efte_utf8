...
int SubHighlightLine(SubGrammar *g, const char *chars, int len,
    ...
    /* Sublime grammars expect \n at end of line for rules like `match: \n`
     * (e.g., line comments pop on newline). eFTE lines have no trailing \n,
     * so we temporarily append one for matching. The extra byte is invisible
     * to the caller (slot[] has only `len` entries). */
    int mlen = len + 1;
    char *mchars = (char *)malloc(mlen + 1);
    if (!mchars) return 0;
    if (len > 0) memcpy(mchars, chars, len);
    mchars[len] = '\n';
    mchars[mlen] = '\0';
    ...
    while (pos <= mlen) {
        ...
        if (g->ctx[top].includePrototype && g->prototypeIdx >= 0 && top != g->prototypeIdx)
            scanContext(g, g->prototypeIdx, mchars, mlen, pos, best, visited);
        scanContext(g, top, mchars, mlen, pos, best, visited);
        ...
        if (me > ms) {
            ...
            if (changed) {
                ...
                int adv = utf8_seqlen_at(mchars, mlen, ms);
        ...
        if (pos == lastPos) {                        /* loop guard */
            if (++iter > maxIter) {
                if (pos < mlen) {
                    int adv = utf8_seqlen_at(mchars, mlen, pos);

 ...
 *
 * P4 (Perforce) support, modelled after SVN implementation.
 *
 * Base class for all other P4-related classes. This is similar to EMessages
 * - starts P4 and shows its messages in list view.
 */
...
static int P4IgnoreRegexpCount = 0;
static RxNode *P4IgnoreRegexp[MAXREGEXP];

int AddP4IgnoreRegexp(const char *regexp) {
    if (P4IgnoreRegexpCount >= MAXREGEXP) return 0;
    if ((P4IgnoreRegexp[P4IgnoreRegexpCount] = RxCompile(regexp)) == NULL) return 0;
    P4IgnoreRegexpCount++;
    return 1;
}

void FreeP4IgnoreRegexp() {
    while (P4IgnoreRegexpCount--) {
        RxFree(P4IgnoreRegexp[P4IgnoreRegexpCount]);
    }
}

EP4Base::EP4Base(int createFlags, EModel **ARoot, const char *title): EList(createFlags, ARoot, title) {
    LineCount = 0;
    Lines = 0;
    Running = 0;
    BufLen = 0;
...
void EP4Base::AddLine(const char *file, int line, const char* msg, int status) {
    P4Line *l;

    l = (P4Line *)malloc(sizeof(P4Line));
    if (l != 0) {
        ...
        LineCount++;
        Lines = (P4Line **)realloc(Lines, sizeof(P4Line *) * LineCount);
        Lines[LineCount-1] = l;
...
void EP4Base::FindBuffer(int line) {
...
void EP4Base::AssignBuffer(EBuffer *B, int line) {
    assert(line >= 0 && line < LineCount);
...
void EP4Base::FindFileLines(EBuffer *B) {
    char path[MAXPATH];
    char *pos;
    strcpy(path, Directory);
    Slash(path, 1);
...
void EP4Base::NotifyDelete(EModel *Deleting) {
    for (int i = 0;i < LineCount;i++) {
...
int EP4Base::GetLine(char *Line, int max) {
    int rc;
    char *p;
    int l;
...
void EP4Base::ParseLine(const char *line, int) {
    AddLine(0, -1, line);
}

void EP4Base::NotifyPipe(int APipeId) {
    if (APipeId == PipeId) {
        char line[1024];
        RxMatchRes RM;
        int i;
        ...
        while (GetLine((char *)line, sizeof(line))) {
            int len = strlen(line);
            if (len > 0 && line[len-1] == '\n') line[--len] = 0;
            for (i = 0;i < P4IgnoreRegexpCount;i++)
                if (RxExec(P4IgnoreRegexp[i], line, len, line, &RM) == 1) break;
            if (i == P4IgnoreRegexpCount) ParseLine(line, len);
        }
        if (!Running) {
            char s[30];
...
int EP4Base::RunPipe(const char *ADir, const char *ACommand, const char *AOnFiles) {
    free(Command);
    free(Directory);
    free(OnFiles);
...

    {
        ...
        sprintf(s, "[running p4 in '%s']", Directory);
        AddLine(0, -1, s);
    }
...
void EP4Base::ClosePipe() {
    ReturnCode = gui->ClosePipe(PipeId);
    PipeId = -1;
    Running = 0;
}

void EP4Base::DrawLine(PCell B, int Line, int Col, ChColor color, int Width) {
    if (Line < LineCount)
...
char *EP4Base::FormatLine(int Line) const {
    if (Line < LineCount) return strdup(Lines[Line]->Msg);
    else return 0;
}

void EP4Base::UpdateList() {
    if (LineCount <= Row || Row >= Count - 1) Row = LineCount - 1;
    Count = LineCount;
    EList::UpdateList();
}

int EP4Base::Activate(int No) {
    ShowLine(View, No);
    return 1;
}

int EP4Base::CanActivate(int Line) const {
    return Line < LineCount && Lines[Line]->File;
}

int EP4Base::IsHilited(int Line) const {
    return Line < LineCount && (Lines[Line]->Status&1);
}

int EP4Base::IsMarked(int Line) const {
    return Line < LineCount && (Lines[Line]->Status&2);
}

int EP4Base::Mark(int Line) {
    if (Line < LineCount) {
...
int EP4Base::ExecCommand(int Command, ExState &State) {
    switch (Command) {
    case ExChildClose:
...
void EP4Base::ShowLine(EView *V, int line) {
...
EEventMap *EP4Base::GetEventMap() {
    return FindEventMap("P4BASE");
}

// Shown in "Closing xxx..." message when closing model
void EP4Base::GetName(char *AName, int MaxLen) const {
    strncpy(AName, Title, MaxLen);
}

// Shown in buffer list
void EP4Base::GetInfo(char *AInfo, int MaxLen) const {
    char format[128];
...
void EP4Base::GetPath(char *APath, int MaxLen) const {
...
void EP4Base::GetTitle(char *ATitle, int MaxLen, char *ASTitle, int SMaxLen) const {
    char format[128];

 ...
 *
 * S.Pinigin copy o_cvsbase.cpp and replace cvs/Cvs/CVS to git/Git/GIT.
 *
 * Base class for all other GIT-related classes. This is similar to EMessages
 * - starts GIT and shows its messages in list view.
 */
...
static int GitIgnoreRegexpCount = 0;
static RxNode *GitIgnoreRegexp[MAXREGEXP];

int AddGitIgnoreRegexp(const char *regexp) {
    if (GitIgnoreRegexpCount >= MAXREGEXP) return 0;
    if ((GitIgnoreRegexp[GitIgnoreRegexpCount] = RxCompile(regexp)) == NULL) return 0;
    GitIgnoreRegexpCount++;
    return 1;
}

void FreeGitIgnoreRegexp() {
    while (GitIgnoreRegexpCount--) {
        RxFree(GitIgnoreRegexp[GitIgnoreRegexpCount]);
    }
}

EGitBase::EGitBase(int createFlags, EModel **ARoot, const char *title): EList(createFlags, ARoot, title) {
    LineCount = 0;
    Lines = 0;
    Running = 0;
    BufLen = 0;
...
void EGitBase::AddLine(const char *file, int line, const char* msg, int status) {
    GitLine *l;

    l = (GitLine *)malloc(sizeof(GitLine));
    if (l != 0) {
        ...
        LineCount++;
        Lines = (GitLine **)realloc(Lines, sizeof(GitLine *) * LineCount);
        Lines[LineCount-1] = l;
...
void EGitBase::FindBuffer(int line) {
...
void EGitBase::AssignBuffer(EBuffer *B, int line) {
    assert(line >= 0 && line < LineCount);
...
void EGitBase::FindFileLines(EBuffer *B) {
    char path[MAXPATH];
    char *pos;
    strcpy(path, Directory);
    Slash(path, 1);
...
void EGitBase::NotifyDelete(EModel *Deleting) {
    for (int i = 0;i < LineCount;i++) {
...
int EGitBase::GetLine(char *Line, int max) {
    int rc;
    char *p;
    int l;
...
void EGitBase::ParseLine(const char *line, int) {
    AddLine(0, -1, line);
}

void EGitBase::NotifyPipe(int APipeId) {
    if (APipeId == PipeId) {
        char line[1024];
        RxMatchRes RM;
        int i;
        ...
        while (GetLine((char *)line, sizeof(line))) {
            int len = strlen(line);
            if (len > 0 && line[len-1] == '\n') line[--len] = 0;
            for (i = 0;i < GitIgnoreRegexpCount;i++)
                if (RxExec(GitIgnoreRegexp[i], line, len, line, &RM) == 1) break;
            if (i == GitIgnoreRegexpCount) ParseLine(line, len);
        }
        if (!Running) {
            char s[30];
...
int EGitBase::RunPipe(const char *ADir, const char *ACommand, const char *AOnFiles) {
    free(Command);
    free(Directory);
    free(OnFiles);
...

    {
        ...
        sprintf(s, "[running git in '%s']", Directory);
        AddLine(0, -1, s);
    }
...
void EGitBase::ClosePipe() {
    ReturnCode = gui->ClosePipe(PipeId);
    PipeId = -1;
    Running = 0;
}

void EGitBase::DrawLine(PCell B, int Line, int Col, ChColor color, int Width) {
    if (Line < LineCount)
...
char *EGitBase::FormatLine(int Line) const {
    if (Line < LineCount) return strdup(Lines[Line]->Msg);
    else return 0;
}

void EGitBase::UpdateList() {
    if (LineCount <= Row || Row >= Count - 1) Row = LineCount - 1;
    Count = LineCount;
    EList::UpdateList();
}

int EGitBase::Activate(int No) {
    ShowLine(View, No);
    return 1;
}

int EGitBase::CanActivate(int Line) const {
    return Line < LineCount && Lines[Line]->File;
}

int EGitBase::IsHilited(int Line) const {
    return Line < LineCount && (Lines[Line]->Status&1);
}

int EGitBase::IsMarked(int Line) const {
    return Line < LineCount && (Lines[Line]->Status&2);
}

int EGitBase::Mark(int Line) {
    if (Line < LineCount) {
...
int EGitBase::ExecCommand(int Command, ExState &State) {
    switch (Command) {
    case ExChildClose:
...
void EGitBase::ShowLine(EView *V, int line) {
...
EEventMap *EGitBase::GetEventMap() {
    return FindEventMap("GITBASE");
}

// Shown in "Closing xxx..." message when closing model
void EGitBase::GetName(char *AName, int MaxLen) const {
    strncpy(AName, Title, MaxLen);
}

// Shown in buffer list
void EGitBase::GetInfo(char *AInfo, int MaxLen) const {
    char format[128];
...
void EGitBase::GetPath(char *APath, int MaxLen) const {
...
void EGitBase::GetTitle(char *ATitle, int MaxLen, char *ASTitle, int SMaxLen) const {
    char format[128];

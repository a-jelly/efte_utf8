 ...
 *
 * S.Pinigin copy o_cvslog.cpp and replace cvs/Cvs/CVS to git/Git/GIT.
 *
 * Subclass of EBuffer for writing log for GIT commit. Creates temporary file
...
EGitLog::EGitLog(int createFlags, EModel **ARoot, const char *Directory, const char *AOnFiles): EBuffer(createFlags, ARoot, NULL) {
    int i, j, p;
    ...
    sprintf(msgFile, "/tmp/efte%d-git-msg", getpid());
#else
    tmpnam(msgFile);
#endif
    SetFileName(msgFile, GitLogMode);

    // Preload buffer with info
    InsertLine(0, 0, "");
    InsertLine(1, 60, "GIT: -------------------------------------------------------");
    InsertLine(2, 59, "GIT: Enter log. Lines beginning with 'GIT:' will be removed");
    InsertLine(3, 4, "GIT:");
    InsertLine(4, 18, "GIT: Commiting in ");
    ...
    if (OnFiles[0]) {
        ...
        while (1) {
            if (OnFiles[i] == 0 || OnFiles[i] == ' ') {
                ...
                char c = OnFiles[i];
                OnFiles[i] = 0;
                status[j] = GitView->GetFileStatus(OnFiles + position[j]);
        ...
        InsertLine(5, 4, "GIT:");
        InsertLine(6, 30, "GIT: Commiting whole directory");
        p = 7;
    }
    InsertLine(p, 4, "GIT:");
    InsertLine(p + 1, 60, "GIT: -------------------------------------------------------");
    SetPos(0, 0);
    FreeUndo();
    free(OnFiles);
    Modified = 0;
...
void EGitLog::ListFiles(int &p, const int fCount, const char *title, const int cnt, const int *position,
...
EViewPort *EGitLog::CreateViewPort(EView *V) {
    V->Port = new EEditPort(this, V);
    AddView(V);
    return V->Port;
}

int EGitLog::CanQuit() const {
    return 0;
}

int EGitLog::ConfQuit(GxView *V, int /*multiFile*/) {
    int i;

    switch (V->Choice(GPC_ERROR, "GIT commit pending", 3, "C&ommit", "&Discard", "&Cancel", "")) {
    case 0: // Commit
        // First save - this is just try
        if (Save() == 0) return 0;
        // Now remove GIT: lines and really save
        for (i = 0;i < RCount;) {
            PELine l = RLine(i);
            if (l->Count >= 4 && strncmp(l->Chars, "GIT:", 4) == 0) DelLine(i);
            else i++;
        }
        Save();
        // DoneCommit returns 0 if OK
        return !GitView->DoneCommit(1);
    case 1: // Discard
        GitView->DoneCommit(0);
        return 1;
    case 2: // Cancel
    default:
        return 0;
...
void EGitLog::GetName(char *AName, int MaxLen) const {
    strncpy(AName, "GIT log", MaxLen);
}

// Shown in buffer list
void EGitLog::GetInfo(char *AInfo, int /*MaxLen*/) const {
    sprintf(AInfo, "%2d %04d:%03d%cGIT log: %-140s", ModelNo, 1 + CP.Row, 1 + CP.Col, Modified ? '*' : ' ', FileName);
}

// Normal and short title (normal for window, short for icon in X)
void EGitLog::GetTitle(char *ATitle, int MaxLen, char *ASTitle, int SMaxLen) const {
    strncpy(ATitle, "GIT log", MaxLen);
    strncpy(ASTitle, "GIT log", SMaxLen);
}

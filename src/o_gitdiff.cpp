 ...
 *
 * S.Pinigin copy o_cvsdiff.cpp and replace cvs/Cvs/CVS to git/Git/GIT.
 *
...
EGitDiff       *GitDiffView = 0;

EGitDiff::EGitDiff(int createFlags, EModel ** ARoot, const char *ADir, const char *ACommand,
                   char *AOnFiles):
        EGitBase(createFlags, ARoot, "GIT diff") {
    GitDiffView = this;
    CurrFile = 0;
    CurrLine = 0;
    ToLine = 0;
...
void
EGitDiff::ParseFromTo(const char *line, int /*len */) {
    const char           *start;
    char           *end;
...
void
EGitDiff::ParseLine(const char *line, int len) {
...
int
EGitDiff::RunPipe(const char *ADir, const char *ACommand, const char *AOnFiles) {
    FreeLines();
    free(CurrFile);
    CurrLine = ToLine = InToFile = 0;
    CurrFile = 0;
...
int
EGitDiff::ExecCommand(int Command, ExState & State) {
    switch (Command) {
    case ExBlockCopy:
        return BlockCopy(0);
...
int EGitDiff::GetContext() const {
    return CONTEXT_GITDIFF;
}

// Event map - this name is used in config files when defining eventmap
EEventMap      *
EGitDiff::GetEventMap() {
    return FindEventMap("GITDIFF");
}

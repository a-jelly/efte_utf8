 ...
 *
 * S.Pinigin copy o_cvsdiff.cpp and replace cvs/Cvs/CVS to p4/P4/P4.
 *
...
EP4Diff       *P4DiffView = 0;

EP4Diff::EP4Diff(int createFlags, EModel ** ARoot, const char *ADir, const char *ACommand,
                   char *AOnFiles):
        EP4Base(createFlags, ARoot, "P4 diff") {
    P4DiffView = this;
    CurrFile = 0;
    CurrLine = 0;
    ToLine = 0;
...
void
EP4Diff::ParseFromTo(const char *line, int /*len */) {
    const char           *start;
    char           *end;
...
void
EP4Diff::ParseLine(const char *line, int len) {
...
int
EP4Diff::RunPipe(const char *ADir, const char *ACommand, const char *AOnFiles) {
    FreeLines();
    free(CurrFile);
    CurrLine = ToLine = InToFile = 0;
    CurrFile = 0;
...
int
EP4Diff::ExecCommand(int Command, ExState & State) {
    switch (Command) {
    case ExBlockCopy:
        return BlockCopy(0);
...
int EP4Diff::GetContext() const {
    return CONTEXT_P4DIFF;
}

// Event map - this name is used in config files when defining eventmap
EEventMap      *
EP4Diff::GetEventMap() {
    return FindEventMap("P4DIFF");
}
